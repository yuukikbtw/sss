...
async function toggleDay(habitId, date) {
    ...
    try {
        ...
        if (response.ok) {
            const todayStr = getTodayString(); // Локальний час!
...
// ============================================
// СИСТЕМА ДОЗВОЛІВ ДЛЯ ДАТЧИКА РУХУ
// ============================================

// Стан дозволу на датчик
...
function detectPlatform() {
    if (window.Capacitor?.isNativePlatform && window.Capacitor.isNativePlatform()) {
        ...
        motionPermissionState.platform = isIOS ? 'ios' : 'android';
    } else {
        const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || 
                      (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
        motionPermissionState.platform = isIOS ? 'ios-web' : 'web';
    }
    console.log('[MOTION] 📱 Платформа:', motionPermissionState.platform);
    return motionPermissionState.platform;
}

/**
 * Перевіряє поточний стан дозволу
 */
async function checkMotionPermission() {
    console.log('[MOTION] 🔍 Перевірка стану дозволу...');
    
    const platform = detectPlatform();
    
    // Android Native - перевіряємо через BackgroundStepCounter
    if (platform === 'android' && BackgroundStepCounter) {
        try {
            const availability = await BackgroundStepCounter.isAvailable();
            console.log('[MOTION] Android availability:', availability);
            
            // Перевіряємо чи сервіс вже запущений
            const running = await BackgroundStepCounter.isRunning();
            if (running.running) {
                motionPermissionState.granted = true;
                motionPermissionState.checked = true;
                console.log('[MOTION] ✅ Android: дозвіл вже надано (сервіс працює)');
                return true;
            }
            
            // Перевіряємо дозволи
            if (window.Capacitor?.Plugins?.BackgroundStepCounter?.checkPermissions) {
                const perms = await BackgroundStepCounter.checkPermissions();
                motionPermissionState.granted = perms.granted === true;
                motionPermissionState.checked = true;
                console.log('[MOTION] Android permissions:', perms);
                return motionPermissionState.granted;
            }
        } catch (e) {
            console.warn('[MOTION] Android check error:', e);
        }
    }
    
    // iOS - перевіряємо DeviceMotionEvent
    if (platform === 'ios' || platform === 'ios-web') {
        // На iOS неможливо перевірити дозвіл без запиту
        // Спробуємо створити тестовий listener
        if (typeof DeviceMotionEvent !== 'undefined') {
            if (typeof DeviceMotionEvent.requestPermission === 'function') {
                // iOS 13+ - потрібен явний запит, перевіряємо localStorage
                const stored = localStorage.getItem('motionPermissionGranted');
                if (stored === 'true') {
                    motionPermissionState.granted = true;
                    motionPermissionState.checked = true;
                    console.log('[MOTION] ✅ iOS: дозвіл збережено як granted');
                    return true;
                }
                console.log('[MOTION] ⚠️ iOS: потрібен запит дозволу (user gesture)');
                return false;
            } else {
                // Старіша iOS або Android - дозвіл не потрібен
                motionPermissionState.granted = true;
                motionPermissionState.checked = true;
                console.log('[MOTION] ✅ DeviceMotion доступний без дозволу');
                return true;
            }
        }
    }
    
    // Web - перевіряємо доступність API
    if (platform === 'web') {
        if (typeof Accelerometer !== 'undefined') {
            try {
                // Перевіряємо Permission API
                const result = await navigator.permissions.query({ name: 'accelerometer' });
                motionPermissionState.granted = result.state === 'granted';
                motionPermissionState.denied = result.state === 'denied';
                motionPermissionState.checked = true;
                console.log('[MOTION] Web accelerometer permission:', result.state);
                return motionPermissionState.granted;
            } catch (e) {
                // Permission API не підтримується для accelerometer
                motionPermissionState.granted = true;
                motionPermissionState.checked = true;
                return true;
            }
        ...
        if (typeof DeviceMotionEvent !== 'undefined') {
            motionPermissionState.granted = true;
            motionPermissionState.checked = true;
            console.log('[MOTION] ✅ Web DeviceMotion доступний');
            return true;
        }
    }
    
    console.log('[MOTION] ❌ Датчик руху недоступний');
    return false;
}

/**
 * Запитує дозвіл на датчик руху (головна функція)
 */
async function requestMotionPermission() {
    console.log('[MOTION] 🔐 ========================================');
    console.log('[MOTION] 🔐 Запит дозволу на датчик руху...');
    console.log('[MOTION] 🔐 ========================================');
    ...
    try {
        ...
        // ============================================
        if (platform === 'android' && BackgroundStepCounter) {
            console.log('[MOTION] 📱 Android: запит через BackgroundStepCounter...');
            
            try {
                // Спочатку перевіряємо доступність
                const availability = await BackgroundStepCounter.isAvailable();
                console.log('[MOTION] Availability:', availability);
                
                if (!availability.available) {
                    console.error('[MOTION] ❌ Датчик недоступний на цьому пристрої');
                    showError(t('sensorNotAvailable') || '❌ Датчик руху недоступний на цьому пристрої');
                    motionPermissionState.denied = true;
                    updateMotionPermissionBanner();
                    return false;
                }
                
                // Запитуємо дозволи
                console.log('[MOTION] Requesting permissions...');
                const permissions = await BackgroundStepCounter.requestPermissions();
                console.log('[MOTION] Permission result:', permissions);
                
                if (permissions.granted) {
                    console.log('[MOTION] ✅ Android дозвіл надано!');
                    motionPermissionState.granted = true;
                    motionPermissionState.denied = false;
                    
                    // Запускаємо сервіс
                    await BackgroundStepCounter.start();
                    ...
                    hideMotionPermissionBanner();
                    updateStepCounterUI();
                    return true;
                } else {
                    console.error('[MOTION] ❌ Android дозвіл відхилено');
                    motionPermissionState.denied = true;
                ...
                console.error('[MOTION] Android permission error:', e);
        ...
        if ((platform === 'ios' || platform === 'ios-web') && 
            ...
            console.log('[MOTION] 🍎 iOS: запит через DeviceMotionEvent.requestPermission()...');
            
            try {
                const permission = await DeviceMotionEvent.requestPermission();
                console.log('[MOTION] iOS permission result:', permission);
                
                if (permission === 'granted') {
                    console.log('[MOTION] ✅ iOS дозвіл надано!');
                    motionPermissionState.granted = true;
                    motionPermissionState.denied = false;
                    ...
                    console.error('[MOTION] ❌ iOS дозвіл відхилено:', permission);
                    motionPermissionState.denied = true;
                    localStorage.setItem('motionPermissionGranted', 'false');
                    showError(t('motionPermissionDenied') || '❌ Доступ до датчика руху заборонено');
                    updateMotionPermissionBanner();
                    return false;
                }
            } catch (e) {
                console.error('[MOTION] iOS permission error:', e);
                motionPermissionState.denied = true;
                showError(t('motionPermissionError') || '❌ Помилка запиту дозволу');
                updateMotionPermissionBanner();
                return false;
            }
        }
        ...
        // ============================================
        // WEB (без явного дозволу)
        // ============================================
        if (typeof DeviceMotionEvent !== 'undefined') {
            console.log('[MOTION] 🌐 Web: DeviceMotion доступний без дозволу');
            motionPermissionState.granted = true;
            stepCounter.isSupported = true;
            startDeviceMotionStepCounter();
            hideMotionPermissionBanner();
        ...
        if (typeof Accelerometer !== 'undefined') {
            console.log('[MOTION] 🌐 Web: Accelerometer API доступний');
            motionPermissionState.granted = true;
            stepCounter.isSupported = true;
        ...
        console.error('[MOTION] ❌ Жоден датчик руху недоступний');
        showError(t('sensorNotAvailable') || '❌ Датчик руху недоступний');
        return false;
        
    } catch (e) {
        console.error('[MOTION] ❌ Помилка запиту дозволу:', e);
        showError(t('motionPermissionError') || '❌ Помилка запиту дозволу');
        return false;
    }
}
...
/**
 * Відкриває системні налаштування дозволів
 */
async function openMotionPermissionSettings() {
    console.log('[MOTION] ⚙️ Відкриття налаштувань дозволів...');
    
    const platform = detectPlatform();
    
    if (platform === 'android') {
        // Android - використовуємо нативний метод плагіна
        if (BackgroundStepCounter?.openSettings) {
            try {
                await BackgroundStepCounter.openSettings();
                console.log('[MOTION] ✅ Налаштування відкрито');
                return;
            } catch (e) {
                console.warn('[MOTION] openSettings failed:', e);
            }
        }
        
        // Альтернатива - через App plugin
        if (window.Capacitor?.Plugins?.App) {
            try {
                await window.Capacitor.Plugins.App.openUrl({
                    url: 'package:com.habittracker.app'
                });
                return;
            } catch (e) {
                console.warn('[MOTION] App.openUrl failed:', e);
            }
        }
        
        // Fallback - показуємо інструкцію
        showError(
            t('openSettingsManually') || 
            '⚙️ Відкрийте Налаштування → Застосунки → Трекер звичок → Дозволи'
        );
    } else if (platform === 'ios' || platform === 'ios-web') {
        // iOS - спробуємо відкрити налаштування
        if (window.Capacitor?.Plugins?.App) {
            try {
                await window.Capacitor.Plugins.App.openUrl({
                    url: 'app-settings:'
                ...
                console.warn('[MOTION] iOS settings failed:', e);
            }
        }
        
        showError(
            t('openSettingsManually') || 
            '⚙️ Відкрийте Налаштування → Конфіденційність → Рух і фітнес'
        );
    } else {
        // Web - показуємо інструкцію
        showError(
            t('browserSettingsInfo') || 
            '⚙️ Перезавантажте сторінку і дозвольте доступ до датчиків'
        );
    }
}

/**
 * Приховує банер з дозволом
 */
function hideMotionPermissionBanner() {
    const banner = document.querySelector('.motion-permission-banner');
    if (banner) {
        banner.style.display = 'none';
        console.log('[MOTION] 🙈 Банер приховано');
    }
}

/**
 * Оновлює банер з дозволом на основі стану
 */
function updateMotionPermissionBanner() {
    const banner = document.querySelector('.motion-permission-banner');
    if (!banner) return;
    ...
    // Якщо дозвіл відхилено - показуємо кнопку налаштувань
    if (motionPermissionState.denied) {
        banner.innerHTML = `
            <p style="margin: 0 0 12px 0; font-size: 0.95rem; color: var(--text-muted);">
                ⚠️ ${t('motionPermissionDeniedDesc') || 'Доступ до датчика руху заборонено. Увімкніть його в налаштуваннях.'}
            </p>
            <button class="btn btn-primary" onclick="openMotionPermissionSettings()" style="width: 100%;">
                ⚙️ ${t('openSettings') || 'Відкрити налаштування'}
            </button>
        `;
    }
}
...
/**
 * Додає банер з проханням надати дозвіл
 */
function addStepCounterPermissionButton() {
    console.log('[MOTION] 📋 Додавання банера дозволів...');
    
    const stepCounterSection = document.getElementById('stepCounterSection');
    if (!stepCounterSection) {
        console.warn('[MOTION] stepCounterSection не знайдено');
        return;
    }
    
    // Видаляємо старий банер якщо є
    const existingBanner = stepCounterSection.querySelector('.motion-permission-banner');
    if (existingBanner) {
        existingBanner.remove();
    }
    
    // Якщо дозвіл вже надано - не показуємо банер
    if (motionPermissionState.granted) {
        console.log('[MOTION] Дозвіл вже надано, банер не потрібен');
        return;
    }
    
    const banner = document.createElement('div');
    banner.className = 'motion-permission-banner';
    ...
    // Різний текст залежно від стану
    if (motionPermissionState.denied) {
        banner.innerHTML = `
            <p style="margin: 0 0 12px 0; font-size: 0.95rem; color: var(--text-muted);">
                ⚠️ ${t('motionPermissionDeniedDesc') || 'Доступ до датчика руху заборонено. Увімкніть його в налаштуваннях.'}
        ...
        banner.innerHTML = `
            ...
            <button class="btn btn-primary" id="enableMotionSensorBtn" style="width: 100%;">
    ...
    if (container) {
        ...
        stepCounterSection.insertBefore(banner, stepCounterSection.firstChild);
    }
    
    // Додаємо обробник кліку через addEventListener (а не onclick)
    const enableBtn = banner.querySelector('#enableMotionSensorBtn');
    if (enableBtn) {
        enableBtn.addEventListener('click', async function(e) {
            console.log('[MOTION] 🖱️ Кнопка натиснута!');
            e.preventDefault();
            
            // Показуємо що йде обробка
            this.disabled = true;
            this.textContent = '⏳ Запит дозволу...';
            
            try {
                const result = await requestMotionPermission();
                console.log('[MOTION] Результат запиту:', result);
                
                if (!result) {
                    this.disabled = false;
                    this.textContent = `🔓 ${t('enableMotionSensor') || 'Увімкнути датчик'}`;
                }
            } catch (e) {
                console.error('[MOTION] Помилка:', e);
                this.disabled = false;
                this.textContent = `🔓 ${t('enableMotionSensor') || 'Увімкнути датчик'}`;
            }
        });
    ...
    console.log('[MOTION] ✅ Банер додано');
}
...
async function initStepCounter() {
    ...
    // Нативний сервіс має свій localStorage і сам скидає при новому дні
    if (window.Capacitor?.isNativePlatform && window.Capacitor.isNativePlatform()) {
    ...
    // Для веб-версії: Завантажуємо збережені дані
    try {
    ...
    // Перевірка нового дня (тільки для веб)
    const today = getStepTodayKey();
    if (stepCounter.lastReset !== today) {
    ...
    console.log('[STEP] 🔄 Ініціалізація веб-крокоміра...');
...
async function initNativeStepCounter() {
    console.log('[STEP] 🤖 ========================================');
    console.log('[STEP] 🤖 Ініціалізація нативного крокоміра...');
    console.log('[STEP] 🤖 ========================================');
    
    // СПОЧАТКУ завантажуємо збережені кроки (щоб не втратити при перезапуску)
    try {
        const stored = localStorage.getItem('stepCounter');
        if (stored) {
            const parsed = JSON.parse(stored);
            const today = getTodayString();
            // Перевіряємо чи це сьогоднішні кроки
            if (parsed.lastReset === today) {
                stepCounter.steps = parsed.steps || 0;
                stepCounter.lastReset = parsed.lastReset;
                console.log('[STEP] 📂 Завантажено збережені кроки:', stepCounter.steps);
            } else {
                console.log('[STEP] 📅 Новий день, скидаємо збережені кроки');
                stepCounter.steps = 0;
                stepCounter.lastReset = today;
            }
        }
    } catch (e) {
        console.warn('[STEP] ⚠️ Помилка завантаження збережених кроків:', e);
    }
    ...
    try {
        ...
        if (!availability.available) {
            console.warn('[STEP] ⚠️ Нативний крокомір недоступний на цьому пристрої');
            motionPermissionState.denied = true;
            showInfo('⚠️ Крокомір недоступний на цьому пристрої');
            return;
        }
        
        // СПОЧАТКУ перевіряємо чи дозволи вже надані
        console.log('[STEP] 🔐 Перевірка дозволів...');
        let permissions = await BackgroundStepCounter.checkPermissions();
        console.log('[STEP] 🔐 Check permissions result:', permissions);
        
        // Якщо дозволи ще не надані - запитуємо через системний діалог
        if (!permissions.granted) {
            console.log('[STEP] 🔐 Дозволи не надані, запитуємо...');
            // Системний діалог з'явиться автоматично
            permissions = await BackgroundStepCounter.requestPermissions();
            console.log('[STEP] 🔐 Request permissions result:', permissions);
        ...
        if (!permissions.granted) {
            ...
            showInfo('❌ Для крокоміра потрібен дозвіл на відстеження активності');
            return;
        }
        
        // Дозвіл надано!
        console.log('[STEP] ✅ Дозволи надані!');
        motionPermissionState.granted = true;
        motionPermissionState.denied = false;
        hideMotionPermissionBanner();
        ...
        try {
            ...
            console.log('[STEP] 🚀 Start result:', startResult);
            ...
            if (startResult.started) {
                ...
                console.error('[STEP] ❌ Сервіс не запустився:', startResult.error || 'невідома помилка');
            }
        } catch (startError) {
            console.error('[STEP] ❌ Помилка запуску сервісу:', startError);
        }
        
        // Отримуємо поточні кроки
        const stepsData = await BackgroundStepCounter.getSteps();
        if (stepsData && stepsData.steps >= 0) {
            // Беремо максимум між збереженим і отриманим (щоб не втратити кроки)
            const savedSteps = stepCounter.steps || 0;
            stepCounter.steps = Math.max(stepsData.steps, savedSteps);
            stepCounter.lastReset = stepsData.date;
            updateStepCounterUI();
            saveStepCounter();
            console.log('[STEP] 📊 Завантажено кроків:', stepCounter.steps, '(сервіс:', stepsData.steps, ', збережено:', savedSteps, ')');
        }
        
        // Слухаємо оновлення кроків (real-time)
        BackgroundStepCounter.addListener('stepUpdate', (data) => {
            console.log('[STEP] 📊 Step update (live):', data.steps);
            if (data.steps !== undefined && data.steps > 0) {
                // Тільки оновлюємо якщо кроків більше (не скидаємо)
                if (data.steps >= stepCounter.steps) {
        ...
        // Періодичний опрос кроків (fallback якщо події не доходять)
        // Кожні 10 секунд перевіряємо кроки
        if (window.stepPollingInterval) {
            clearInterval(window.stepPollingInterval);
        }
        window.stepPollingInterval = setInterval(async () => {
            try {
                const data = await BackgroundStepCounter.getSteps();
                // ВАЖЛИВО: не скидаємо кроки якщо сервіс повернув менше!
                if (data && data.steps !== undefined && data.steps > stepCounter.steps) {
                    console.log('[STEP] 📊 Polling update:', stepCounter.steps, '->', data.steps);
                    stepCounter.steps = data.steps;
                    saveStepCounter();
                    updateStepCounterUI();
                }
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('[APP] 🚀 DOMContentLoaded - старт ініціалізації');
    
    initCategories();
    initUserProgress(); 
    
    // checkAuth може бути довгим - не чекаємо
    checkAuth(); 
    
    // Ініціалізуємо крокомір паралельно (не чекаємо на checkAuth)
    try {
        // Чекаємо на готовність Capacitor якщо це нативна платформа
        if (window.Capacitor?.isNativePlatform && window.Capacitor.isNativePlatform()) {
            console.log('[APP] ⏳ Чекаємо на готовність Capacitor...');
            await new Promise(resolve => setTimeout(resolve, 300));
        }
        
        console.log('[APP] 🦶 Запускаємо initStepCounter...');
        await initStepCounter();
        console.log('[APP] ✅ initStepCounter завершено');
    } catch (e) {
        console.error('[APP] ❌ Помилка initStepCounter:', e);
    }
...
function isHabitCompletedToday(habitId) {
    const today = getTodayString(); // Локальний час!
 ...
 * Запустити у console: fullDiagnostic()
 */
async function fullDiagnostic() {
    console.log('');
    console.log('═'.repeat(60));
    console.log('🔧 HABIT TRACKER - FULL SYSTEM DIAGNOSTIC');
    console.log('═'.repeat(60));
    console.log('');

    // 1. ДАТА/ЧАС
    console.group('📅 ДАТА І ЧАС');
    const now = new Date();
    console.log('Локальний час:', now.toLocaleString());
    console.log('UTC час:', now.toISOString());
    console.log('getTodayString():', getTodayString());
    console.log('toISOString().split("T")[0]:', new Date().toISOString().split('T')[0]);
    console.log('Різниця (хвилин):', -now.getTimezoneOffset());
    if (getTodayString() !== new Date().toISOString().split('T')[0]) {
        console.warn('⚠️ УВАГА: Локальна дата відрізняється від UTC! Це нормально якщо після півночі.');
    }
    console.groupEnd();
    console.log('');

    // 2. ПЛАТФОРМА
    console.group('📱 ПЛАТФОРМА');
    console.log('Capacitor:', window.Capacitor ? '✅ Так' : '❌ Ні');
    console.log('isNativePlatform:', window.Capacitor?.isNativePlatform?.() ? '✅ Native' : '❌ Web');
    ...
    // 3. ПЛАГІНИ
    console.group('🔌 ПЛАГІНИ');
    console.log('BackgroundStepCounter:', BackgroundStepCounter ? '✅ Доступний' : '❌ Недоступний');
    console.log('LocalNotifications:', LocalNotifications ? '✅ Доступний' : '❌ Недоступний');
    
    if (window.Capacitor?.Plugins) {
        console.log('Всі плагіни:', Object.keys(window.Capacitor.Plugins).join(', '));
    }
    console.groupEnd();
    console.log('');

    // 4. КРОКОМІР
    console.group('🚶 КРОКОМІР');
    console.log('stepCounter.isSupported:', stepCounter.isSupported);
    console.log('stepCounter.isNative:', stepCounter.isNative);
    console.log('stepCounter.steps:', stepCounter.steps);
    console.log('stepCounter.lastReset:', stepCounter.lastReset);
    console.log('motionPermissionState:', JSON.stringify(motionPermissionState));
    
    if (BackgroundStepCounter) {
        try {
            const avail = await BackgroundStepCounter.isAvailable();
            console.log('BackgroundStepCounter.isAvailable():', JSON.stringify(avail));
            
            const running = await BackgroundStepCounter.isRunning();
            console.log('BackgroundStepCounter.isRunning():', JSON.stringify(running));
            
            const perms = await BackgroundStepCounter.checkPermissions?.() || 'метод недоступний';
            console.log('BackgroundStepCounter.checkPermissions():', JSON.stringify(perms));
        } catch (e) {
            console.error('Помилка перевірки крокоміра:', e.message);
        }
    }
    console.groupEnd();
    console.log('');
    ...
    // 5. СПОВІЩЕННЯ
    console.group('🔔 СПОВІЩЕННЯ');
    console.log('Web Notification API:', typeof Notification !== 'undefined' ? '✅ Так' : '❌ Ні (нормально для WebView)');
    console.log('Web permission:', typeof Notification !== 'undefined' ? Notification.permission : 'N/A (WebView)');
    
    if (LocalNotifications) {
        try {
            const perms = await LocalNotifications.checkPermissions();
            console.log('LocalNotifications permission:', perms.display);
            
            const pending = await LocalNotifications.getPending();
            console.log('Pending notifications:', pending.notifications?.length || 0);
        } catch (e) {
            console.error('Помилка перевірки сповіщень:', e.message);
        }
    }
    console.groupEnd();
    console.log('');
    ...
    // 6. ЗВИЧКИ
    console.group('📋 ЗВИЧКИ');
    console.log('Кількість звичок:', habits.length);
    console.log('Звички з нагадуваннями:', habits.filter(h => h.reminder?.type !== 'none').length);
    
    const todayStr = getTodayString();
    habits.forEach(h => {
        const completedDates = habitCompletedDatesMap.get(h.id);
        const isCompletedToday = completedDates?.has(todayStr);
        console.log(`  • ${h.name}: ${isCompletedToday ? '✅ Виконано' : '⏳ Не виконано'} (dates: ${completedDates?.size || 0})`);
    });
    console.groupEnd();
    console.log('');

    // 7. АВТОРИЗАЦІЯ
    console.group('🔐 АВТОРИЗАЦІЯ');
    console.log('currentUser:', currentUser ? '✅ Авторизований' : '❌ Ні');
    console.log('authToken:', localStorage.getItem('authToken') ? '✅ Є' : '❌ Немає');
    ...
    console.log('  testNotification() - відправити тестове сповіщення');
    console.log('  testScheduledNotification() - сповіщення через 10 сек');
    console.log('  requestMotionPermission() - запитати дозвіл на сенсори');
    console.log('  restartStepCounter() - перезапустити крокомір вручну');
    console.log('═'.repeat(60));
}

/**
 * Ручний перезапуск крокоміра (для діагностики)
 */
async function restartStepCounter() {
    console.log('[STEP] 🔄 Ручний перезапуск крокоміра...');
    
    if (!BackgroundStepCounter) {
        console.error('[STEP] ❌ BackgroundStepCounter недоступний');
        return { success: false, error: 'Plugin not available' };
    }
    
    try {
        // 1. Зупиняємо якщо працює
        console.log('[STEP] Зупиняємо сервіс...');
        await BackgroundStepCounter.stop();
        
        // 2. Перевіряємо дозволи
        const perms = await BackgroundStepCounter.checkPermissions();
        console.log('[STEP] Permissions:', perms);
        
        if (!perms.granted) {
            console.log('[STEP] Запитуємо дозволи...');
            const newPerms = await BackgroundStepCounter.requestPermissions();
            if (!newPerms.granted) {
                console.error('[STEP] ❌ Дозволи не надані');
                return { success: false, error: 'Permissions denied' };
            }
        }
        
        // 3. Запускаємо сервіс
        console.log('[STEP] Запускаємо сервіс...');
        const result = await BackgroundStepCounter.start();
        console.log('[STEP] Start result:', result);
        ...
        if (result.started) {
            ...
            console.log('[STEP] ✅ Крокомір успішно перезапущено!');
            showSuccess('✅ Крокомір запущено!');
            return { success: true };
        } else {
            console.error('[STEP] ❌ Не вдалося запустити:', result.error);
            return { success: false, error: result.error };
        }
    } catch (e) {
        console.error('[STEP] ❌ Помилка:', e);
        return { success: false, error: e.message };
    }
}

// Робимо функції доступними глобально для консолі
window.fullDiagnostic = fullDiagnostic;
window.testNotification = testNotification;
window.testScheduledNotification = testScheduledNotification;
window.diagnosticNotifications = diagnosticNotifications;
window.requestMotionPermission = requestMotionPermission;
window.restartStepCounter = restartStepCounter;

/**
 * Запит на вимкнення оптимізації батареї
 * КРИТИЧНО для OPPO/Realme/Xiaomi!
 */
async function requestBatteryOptimization() {
    console.log('[BATTERY] 🔋 Запит на вимкнення оптимізації батареї...');
    
    if (!BackgroundStepCounter) {
        console.error('[BATTERY] ❌ Plugin недоступний');
        showError('Функція недоступна');
        return;
    }
    
    try {
        const result = await BackgroundStepCounter.requestIgnoreBatteryOptimization();
        console.log('[BATTERY] Результат:', result);
        
        if (result.alreadyIgnoring) {
            showSuccess('✅ Оптимізація батареї вже вимкнена');
        } else {
            showInfo('Будь ласка, дозвольте застосунку працювати без обмежень');
        }
        return result;
    } catch (e) {
        console.error('[BATTERY] ❌ Помилка:', e);
        showError('Не вдалося відкрити налаштування');
    }
}
 ...
 */
async function sensorDiagnostic() {
    console.log('');
    console.log('═'.repeat(60));
    console.log('🔬 ДІАГНОСТИКА СЕНСОРІВ');
    console.log('═'.repeat(60));
    console.log('');
    
    if (!BackgroundStepCounter) {
        console.error('❌ BackgroundStepCounter плагін недоступний');
        return null;
    }
    
    try {
        // Отримуємо діагностику сенсорів
        const sensors = await BackgroundStepCounter.getSensorDiagnostics();
        console.log('📱 Пристрій:', sensors.manufacturer, sensors.model);
        console.log('📱 Android версія:', sensors.androidVersion);
        console.log('');
        console.log('🔍 СЕНСОРИ:');
        console.log('  Step Counter:', sensors.hasStepCounter ? '✅ Є' : '❌ Немає');
        if (sensors.stepCounterName) {
            console.log('    Назва:', sensors.stepCounterName);
            console.log('    Виробник:', sensors.stepCounterVendor);
        }
        console.log('  Step Detector:', sensors.hasStepDetector ? '✅ Є' : '❌ Немає');
        if (sensors.stepDetectorName) {
            console.log('    Назва:', sensors.stepDetectorName);
        }
        console.log('  Accelerometer:', sensors.hasAccelerometer ? '✅ Є' : '❌ Немає');
        console.log('');
        console.log('🔄 Сервіс працює:', sensors.isServiceRunning ? '✅ Так' : '❌ Ні');
        
        // Перевіряємо батарею
        const battery = await BackgroundStepCounter.checkBatteryOptimization();
        console.log('🔋 Оптимізація батареї вимкнена:', battery.isIgnoringBatteryOptimization ? '✅ Так' : '❌ Ні');
        
        if (!battery.isIgnoringBatteryOptimization) {
            console.log('');
            console.log('⚠️ УВАГА: На пристроях OPPO/Realme/Xiaomi оптимізація батареї');
            console.log('   може завадити роботі крокоміра у фоні!');
            console.log('   Запустіть: requestBatteryOptimization()');
        }
        
        // Перевіряємо дозволи
        const perms = await BackgroundStepCounter.checkPermissions();
        console.log('');
        console.log('🔐 Дозвіл на активність:', perms.activityRecognition ? '✅ Надано' : '❌ Не надано');
        
        // Перевіряємо кроки
        const steps = await BackgroundStepCounter.getSteps();
        console.log('');
        console.log('👣 Кроків сьогодні:', steps.steps);
        console.log('📅 Дата:', steps.date);
        
        console.log('');
        console.log('═'.repeat(60));
        console.log('');
        console.log('Щоб виправити проблему:');
        console.log('1. requestBatteryOptimization() - вимкнути оптимізацію батареї');
        console.log('2. restartStepCounter() - перезапустити крокомір');
        console.log('3. Походіть і перевірте чи рахує кроки');
        console.log('═'.repeat(60));
        
        return { sensors, battery, perms, steps };
    } catch (e) {
        console.error('❌ Помилка діагностики:', e);
        return null;
    }
}

window.requestBatteryOptimization = requestBatteryOptimization;
window.sensorDiagnostic = sensorDiagnostic;
