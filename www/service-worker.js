...
self.addEventListener('fetch', (event) => {
  ...
  // Критичні API endpoints - ТІЛЬКИ мережа, без кешу
  // (авторизація, перевірка здоров'я сервера)
  const noCacheEndpoints = ['/api/health', '/api/login', '/api/register', '/api/me', '/api/logout'];
  if (noCacheEndpoints.some(endpoint => url.pathname.startsWith(endpoint))) {
    console.log('🔒 [SW] No-cache endpoint:', url.pathname);
    event.respondWith(fetch(request).catch(err => {
      console.warn('⚠️ [SW] Network error for critical endpoint:', err.message);
  ...
  // Інші API запросы - Network First (сеть приоритетнее)
